...
- `10:00` **用户指令「更新一下文档」→ dev-history 三件套重建 + 推送公开仓库（09-25）。**
  - 用户指令「更新一下文档」→ dev-history 三件套重建 + 推送公开仓库（09-25）。
- `10:01` **合并收口（用户放行）：--no-ff merge docs/dev-history-0925 → main = 15b0**
  - **合并收口（用户放行）**：`--no-ff` merge `docs/dev-history-0925` → main = `15b06f3`（3 文件 +…
- `12:11` **用户指令「打包一起做了吧」→ 三个优化方向同日三个分支（09-25，基 main 15b06f3）：**
  - 用户指令「打包一起做了吧」→ 三个优化方向同日三个分支（09-25，基 main 15b06f3）：
- `12:34` **用户指令「检查环节，云端 3 个分支，有问题修，没问题合并」→ 09-25 三个观测分支检查+合并放行，全流程闭环：**
  - 用户指令「检查环节，云端 3 个分支，有问题修，没问题合并」→ 09-25 三个观测分支检查+合并放行，全流程闭环：
- `15:12` **§38-B 收官 + 09-25 真机验证（用户确认 app 已装最新包 1.0.0+1800）**
  - # §38-B 收官 + 09-25 真机验证（用户确认 app 已装最新包 1.0.0+1800）
- `15:26` **用户报告：记忆编辑模块点键盘时光标/内容跳回最顶端。诊断结论（09-25）：Compose 框架已知 bug，非 app**
  - 用户报告：记忆编辑模块点键盘时光标/内容跳回最顶端。诊断结论（09-25）：Compose 框架已知 bug，非 app 代码错——Google issuetr…
- `15:46` **memory-editor 跳顶修复分支 CI 绿（09-25，停止节点）：fix/memory-editor-jump**
  - **memory-editor 跳顶修复分支 CI 绿（09-25，停止节点）**：`fix/memory-editor-jump-to-top` @ `13d…
- `15:53` **memory-editor-jump-to-top 分支独立复核（09-25 第二轮，应要求重验）：fix/memory**
  - **memory-editor-jump-to-top 分支独立复核（09-25 第二轮，应要求重验）**：`fix/memory-editor-jump-to…
- `16:17` **窗口化编辑分支 CI 绿（09-25 第二轮，停止节点）：fix/memory-editor-jump-to-top @**
  - **窗口化编辑分支 CI 绿（09-25 第二轮，停止节点）**：`fix/memory-editor-jump-to-top` @ `17770a20`（2 …
- `16:17` **memory-editor-windowed-edit 新提交复核（09-25，分支第三轮验证）：fix/memory-**
  - **memory-editor-windowed-edit 新提交复核（09-25，分支第三轮验证）**：`fix/memory-editor-jump-to-…
- `16:22` **用户真机确认：窗口化编辑（fix/memory-editor-jump-to-top @ 17770a20）验证没有问题**
  - 用户真机确认：窗口化编辑（fix/memory-editor-jump-to-top @ 17770a20）验证没有问题——大文件点 Edit 卡顿消失、窗口手…
- `16:23` **memory-editor 分支合并收口（09-25，用户放行）：fix/memory-editor-jump-to-t**
  - **memory-editor 分支合并收口（09-25，用户放行）**：`fix/memory-editor-jump-to-top` 双提交（13d8e9a…
- `16:31` **用户问「应用与最佳实践的差别」→ 全库最佳实践对照（09-25，main 422914d1，/tmp/rk-bp-092**
  - 用户问「应用与最佳实践的差别」→ 全库最佳实践对照（09-25，main 422914d1，/tmp/rk-bp-0925）。外部基准真拉取核对：Android…
- `16:33` **backlog 存量盘点（09-25，应问分析）：1079 行 ~36 条未关项，净「该修未修」只有 13 条 P2（§**
  - backlog 存量盘点（09-25，应问分析）：1079 行 ~36 条未关项，净「该修未修」只有 13 条 P2（§24a/26/27c(1)/27d/27…
- `16:51` **§38A 落地（09-25）：fix/skills-prompt-budget @ 254ecc7（1 文件 +6/−1**
  - §38A 落地（09-25）：`fix/skills-prompt-budget` @ 254ecc7（1 文件 +6/−1，MAX_SKILLS_IN_PRO…
- `17:02` **09-25 应用优化盘点：流式恢复两条待真机验证风险**
  - 读远端 main 422914d 源码发现 PartialStreamRecovery.kt:81-99 的决策顺序有竞态：SKIP_EMPTY / SKIP_…
- `17:36` **检查环节完成（09-25，两分支）：**
  - 检查环节完成（09-25，两分支）：
- `17:39` **§流式恢复竞态修复分支 CI 绿（09-25，停止节点）**
  - # §流式恢复竞态修复分支 CI 绿（09-25，停止节点）
- `17:39` **合并收口（09-25，用户放行，两分支）：--no-ff 按序 merge fix/skills-prompt-budg**
  - 合并收口（09-25，用户放行，两分支）：`--no-ff` 按序 merge `fix/skills-prompt-budget`（254ecc7c）→ 1a…
- `18:00` **09-25 三日日志实读：优化空间有定量候选，不把计数当故障**
  - 读取 /var/minis/logs/minis-{23,24,25}.log、memspike、launch-beacon（应用 INFO 正式行筛选，排除 …
- `18:07` **09-25 真发现：卸载后自动压缩用旧 token 触发，两次可疑冗余**
  - 主线 1cc5ab1d 源码 AgentLoopEngine.kt 356→370 先 `host.offloadContextIfNeeded(window,…
- `18:33` **分支 CI 绿（09-25 停止节点，检查会话）：fix/offload-stale-token-compact @ f**
  - 分支 CI 绿（09-25 停止节点，检查会话）：`fix/offload-stale-token-compact` @ `f87d9102`（1 commit…
- `18:33` **卸载后陈旧令牌修复分支 CI 绿（09-25，停止节点）**
  - `fix/offload-stale-token-compact` @ `f87d9102`（1 commit，4 文件 +48/−8，基点 main 1cc5…
- `18:36` **分支完整核查补跑（09-25，应要求）：上轮只做 diff 走查+CI 两项属实偷懒，补齐三块：①全量 diff 逐行（**
  - 分支完整核查补跑（09-25，应要求）：上轮只做 diff 走查+CI 两项属实偷懒，补齐三块：①全量 diff 逐行（ChatContextWindow 16…
- `18:38` **系统 prompt 9 万字符实测：三条证据链齐，登记不修（09-25）**
  - 用户问"9 万字符先测实际 token/缓存命中再谈精简"应该怎么来 → 实测三步（minis-model-use agentrouter-1/deepseek…
- `18:41` **合并收口（09-25，用户放行）：--no-ff merge fix/offload-stale-token-compa**
  - 合并收口（09-25，用户放行）：`--no-ff` merge `fix/offload-stale-token-compact` @ `f87d9102` …
- `19:09` **对话历史抽屉行精简分支 CI 绿（09-25，停止节点）**
  - # 对话历史抽屉行精简分支 CI 绿（09-25，停止节点）
- `19:14` **drawer-row-slim 分支完整核查（09-25，检查会话，8/8 全跑）：feat/drawer-row-sl**
  - **drawer-row-slim 分支完整核查（09-25，检查会话，8/8 全跑）**：`feat/drawer-row-slim` @ `1a2285b4…
- `19:35` **drawer-row-slim 合并收口（09-25，用户放行）：--no-ff merge feat/drawer-r**
  - **drawer-row-slim 合并收口（09-25，用户放行）**：`--no-ff` merge `feat/drawer-row-slim` @ `1…
- `20:41` **09-25 晚三条 UI 线全部到停止节点（分支 CI 绿，均未合并）：①feat/typing-status-band**
  - **09-25 晚三条 UI 线全部到停止节点（分支 CI 绿，均未合并）**：①`feat/typing-status-band` @ a54c2f78——状…
- `20:43` **09-25 晚收尾（用户明示「合并不在这里」）：本会话停止节点 = 分支 CI 绿为终态，合并由别处发起。两条就绪分支停**
  - **09-25 晚收尾（用户明示「合并不在这里」）**：本会话停止节点 = 分支 CI 绿为终态，合并由别处发起。两条就绪分支停在远端等合并：`feat/typ…
- `20:58` **三分支检查 + drawer-context-menu 合并 + typing-band 返工（09-25）**
  - # 三分支检查 + drawer-context-menu 合并 + typing-band 返工（09-25）
- `21:19` **typing-band-live 合并收口（09-25，用户放行）：8 项固定清单全跑后 --no-ff merge f**
  - **typing-band-live 合并收口（09-25，用户放行）**：8 项固定清单全跑后 `--no-ff` merge `fix/typing-ban…
- `21:28` **分支清理 + 「检查=上全套」规则落地（09-25，用户拍板）：①feat/typing-status-band @ a**
  - **分支清理 + 「检查=上全套」规则落地（09-25，用户拍板）**：①`feat/typing-status-band` @ a54c2f78 API DE…
- `21:46` **typing-band-overlay 回归修复（09-25 深夜，停止节点）：用户报今天合并引入的回归——agent**
  - **typing-band-overlay 回归修复（09-25 深夜，停止节点）**：用户报今天合并引入的回归——agent 执行期间底部 band 持续显示…
- `21:47` **typing-band-overlay 核查 + 合并 main = 50b399c9（09-25，用户放行）**
  - # typing-band-overlay 核查 + 合并 main = 50b399c9（09-25，用户放行）
- `22:04` **09-25 全量回归审计：今天 24 提交 / 33 文件，无功能级 bug，2 条次生开销 P2 + 5 条边界登记**
  - **范围**：基线 `2022b0c5`（昨 21:12）→ main `50b399c9`，`git log` 实测今天进 main 的是 **24 个提交 …
- `22:08` **§6.5 核查清单二次强化：机械化捆绑 + 判断项协议（09-25 晚，用户拍板）**
  - **背景**：用户发现弱模型会偷懒——「声称做了 8 项」但实际只跑 2 项。要求把检查要求硬性写死 + 用设计改进检查步骤。
- `22:15` **rikkaminis-dev-methodology 评审 + 修正（09-25，用户要求复核别处模型的技能优化）**
  - **背景修正**：用户澄清——今天的改动**已经真机验证过**（装的是分支包，不是等 main 的包）。所以上一条审计的"用户手里 1.0.0+1802 不含今…
- `22:31` **rikkaminis-dev-methodology v1.3.1 评审：机械门禁有 7 处已实测漏拦（09-25，只评审未改）**
  - 用户问「这个技能是否完善、能否优化」→ 全文读 + 隔离探针实测（不改源码、不碰真实分支）。**核心判断**：骨架（分支 CI 绿=停止节点 / 合并需用户授权…
- `22:43` **rikkaminis-dev-methodology 第 1 批修复完成（v1.3.1 → v1.3.2，09-25）**
  - **关键环境事实（纠正我上一条的说法）**：`/var/minis/skills` **不在任何 git 仓库里**（`git rev-parse --show…
- `22:58` **★ 技能元数据的真相：应用读的是 skills.db，不是 SKILL.md 的 frontmatter（09-25 用户实证发现）**
  - **用户问「为什么我没看到这个技能对应的改变」——他发现技能显示 version 1.0.0 + 旧的长 description，但正文已经是我今天改过的新内容…
- `23:34` **skill-metadata-resync 核查 + 合并收口（09-25，用户放行）：远端分支 fix/skill-m**
  - **skill-metadata-resync 核查 + 合并收口（09-25，用户放行）**：远端分支 `fix/skill-metadata-resync-…
- `23:51` **09-25 三天改动真机验证闭环（用户确认 1/2/3 已验证）：①skill-metadata-resync（装 +e**
  - **09-25 三天改动真机验证闭环（用户确认 1/2/3 已验证）**：①skill-metadata-resync（装 +ee22115b 包，改 SKIL…

## 2026-09-26（28 条）

- `00:05` **2026-09-26 00:05:00**
- `00:05` **chore/remove-skill-detail-rescan 分支 CI 绿（09-25 深夜，停止节点）：用户发现**
  - **chore/remove-skill-detail-rescan 分支 CI 绿（09-25 深夜，停止节点）**：用户发现技能详情页「从磁盘重新扫描」在 …
- `00:08` **chore/remove-skill-detail-rescan 合并收口（09-26，用户放行）：全套 8 项核查在本**
  - **chore/remove-skill-detail-rescan 合并收口（09-26，用户放行）**：全套 8 项核查在本会话独立复跑（非沿用昨夜结论）—…
- `00:13` **教训（我犯的）：绕过 verify_branch.sh 编排器手跑子集（09-26 复发确认）：合并 chore/rem**
  - **教训（我犯的）：绕过 verify_branch.sh 编排器手跑子集（09-26 复发确认）**：合并 chore/remove-skill-detail…
- `00:19` **remove-skill-detail-rescan 验证闭环（09-26 用户确认）：用户用分支包（68ef2d51**
  - **remove-skill-detail-rescan 验证闭环（09-26 用户确认）**：用户用分支包（68ef2d51 的 CI 产物）真机验证通过——…
- `00:19` **技能元数据回灌修复（09-25 深夜，已进 main）**
  - **根因（三源核实）**：技能元数据存 `skills.db`，模型侧 `<available_skills>` 读**库里的** `description`（…
- `00:25` **2026-09-26 00:25:00**
- `00:27` **dev-history 三件套重建 + 推送（09-26，任务「更新一下文档」）：三个回归测试先过（test_rebui**
  - **dev-history 三件套重建 + 推送（09-26，任务「更新一下文档」）**：三个回归测试先过（test_rebuild/test_sanitize…
- `09:13` **Filterrr/RikkaMinis 仓库调查（09-26，用户问「他改了什么」）：①不是本仓库 fork——API**
  - **Filterrr/RikkaMinis 仓库调查（09-26，用户问「他改了什么」）**：①不是本仓库 fork——API fork=False、compa…
- `09:25` **吸收 Filterrr 改动 → 拆成两个派发任务（09-26，用户决定 A+B，B 整包不拆）：派发文件在 /var/**
  - **吸收 Filterrr 改动 → 拆成两个派发任务（09-26，用户决定 A+B，B 整包不拆）**：派发文件在 `/var/minis/shared/ab…
- `09:59` **会话 A：吸收 Filterrr 网络观测与预热包 → 分支 CI 绿（09-26，停止节点）**
  - **会话 A：吸收 Filterrr 网络观测与预热包 → 分支 CI 绿（09-26，停止节点）**
- `10:33` **B 分支 CI 三轮红→绿收口（09-26 02:30Z，停止节点达成，未合并）**
  - **B 分支 CI 三轮红→绿收口（09-26 02:30Z，停止节点达成，未合并）**
- `11:03` **吸收 Filterrr 两分支最高规格复核（09-26，用户要求「不省」）：报告 /var/minis/shared/a**
  - **吸收 Filterrr 两分支最高规格复核（09-26，用户要求「不省」）**：报告 `/var/minis/shared/absorb-filterrr-…
- `11:35` **工具调用时底部跳动的根因定位（09-26，用户报告「调用工具时跳」）**
  - - 36dp 打字状态带本身是稳的（TypingIndicator 内容包固定 36dp Box，空闲=36dp Spacer，ChatIndicators.k…
- `11:37` **senseaudio.cn deepseek-v4.1-flash 档位缺陷调查（09-26，实测定案，未修）**
  - **用户观察**：api.senseaudio.cn 的 deepseek-v4.1-flash 只显示 低/中/高/自动，直觉不对。**实测确认是双层缺陷，与…
- `11:51` **tool-bar-reserve-frame-skew 修复到停止节点（09-26，用户拍板修）**
  - - **分支**：`fix/tool-bar-reserve-frame-skew` @ `e2ee03a16727c755cd02e017227f49fd25…
- `12:02` **senseaudio 档位缺陷修复收口（09-26，停止节点达成，未合并）：分支 fix/senseaudio-effo**
  - **senseaudio 档位缺陷修复收口（09-26，停止节点达成，未合并）**：分支 `fix/senseaudio-effort-truth` @ `76…
- `13:01` **真机日志取证能力（重大发现，2026-09-26）**
  - **app 的日志在沙箱里直接可读**：`/var/minis/logs/minis-YYYY-MM-DD.log`（主进程）、`minis-YYYY-MM-D…
- `13:25` **两个吸收 Filterrr 分支真机验证结果（09-26，用户确认）**
  - 用户装最新包 v1.0.0+1837（构建自 f2b00151 = 当前 main 最新）真机验证两个吸收能力：
- `13:25` **内容丢失 + 抽屉行跳 修复到停止节点（09-26，用户拍板修，未合并）**
  - **分支** `fix/lost-turn-content-and-row-jump` @ `78d766429cae5b232d5e3cea94f985848…
- `13:54` **★ 更正：/tmp 工作目录消失的真因不是「磁盘压力」，是 app 自己的 1 小时收割（09-26，源码级定案）**
  - **证据链（三源）**：①日志 `I/ExecutionCoordinator: cleanupProotTmp: cleared N aged entries…
- `14:23` **★ /tmp 收割判据修复（09-26，用户拍板「按你的建议改」，已合并 main = c06cf402）**
  - **你的怀疑被证实是机制缺陷**：`cleanupProotTmp` → `sweepGuestTmpDir` 只看条目**自己的 mtime**（>1h 删）…
- `14:40` **装包验证闭环（09-26，用户确认 1/2/4/5 已验证）：main = c06cf402 四项真机验证通过——①工具**
  - **装包验证闭环（09-26，用户确认 1/2/4/5 已验证）**：main = c06cf402 四项真机验证通过——①工具执行期强杀 app 重进，已显示…
- `15:23` **日志分析（09-26，用户指派）→ 报告 /var/minis/shared/log-analysis-0926/REPORT.md**
  - 范围 /var/minis/logs 173M（深挖 09-25/26）；背景=高强度开发期，先剥离开发活动信号。核心结论：
- `15:27` **日志分析报告 0926 核验（09-26，用户指派）→ 报告 /var/minis/shared/log-analysis-0926/REPORT.md（含 §6 核验记录）**
  - **逐条对上**：ERROR 2/8、worker 13 条谱系（4×403+3×429+1×503+5×CANCEL）、beacon 284/91、uptim…
- `15:32` **抽屉第二行长时间空白根因 + 修复到停止节点（09-26，用户拍板修法一，未合并）**
  - **用户观察**：抽屉会话行第二行（预览）会长时间空着——尤其慢模型/长推理时。我先给了错误初判（"运行中短暂，主要是中断尾"），用户纠正：**运行中的会话本身…
- `15:53` **日志报告办法 ①②③ 修复收口（09-26，用户拍板「就在这里做了」）→ 报告 §7 /var/minis/shared/log-analysis-0926/REPORT.md**
  - **① beacon 保留 uptime_was（app 侧，停止节点，未合并）**：分支 `fix/launch-beacon-uptime` @ `593d…
- `16:19` **双分支检查 + 合并 main = 8dcb82e8（09-26，用户「检查完没问题就合并」）**
  - **结局**：`fix/launch-beacon-uptime` @ 593dc5b0 + `fix/preview-null-overwrite` @ 9d…
