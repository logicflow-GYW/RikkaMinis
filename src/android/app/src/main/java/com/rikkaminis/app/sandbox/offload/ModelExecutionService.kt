...
class ModelExecutionService : Service() {
    ...
    private fun executeRun(requestJson: String, dir: File): String {
        ...
        val instance = com.rikkaminis.app.data.model.ProviderInstance(
            ...
            providerType = safeEnum(
                ...
                com.rikkaminis.app.data.model.ProviderType.openAI,
            ),
            credentialType = safeEnum(
                req.optString("credential_type", "apiKey"),
                com.rikkaminis.app.data.model.ProviderCredential.apiKey,
            ),
            ...
            imageEndpointMode = safeEnum(
                ...
                com.rikkaminis.app.data.model.ImageEndpointMode.auto,
            ),
            imageEndpointResolved = req.optString("image_endpoint_resolved", "").let {
                if (it.isNotEmpty()) safeEnumOrNull<com.rikkaminis.app.data.model.ImageEndpointMode>(it) else null
            },
        ...
        val model = com.rikkaminis.app.data.model.LLMModel(
        ...
        val messages = jsonObjList(req.optJSONArray("messages")).map { obj ->
            com.rikkaminis.app.data.model.LLMMessage(
                role = try {
                    com.rikkaminis.app.data.model.LLMMessage.Role.valueOf(
                        obj.getString("role").uppercase()
                    )
                } catch (_: Exception) {
                    com.rikkaminis.app.data.model.LLMMessage.Role.USER
                },
                ...
                audioParts = jsonObjList(obj.optJSONArray("audio_parts")).mapNotNull { a ->
                    ...
                    if (b64.isEmpty()) null
                    else com.rikkaminis.app.data.model.LLMMessage.AudioPart(
        ...
        val imageParts = jsonObjList(req.optJSONArray("image_parts")).map { obj ->
            com.rikkaminis.app.data.model.LLMMessage.ImagePart(
        ...
        if (inputJson.isNotEmpty() && provider is com.rikkaminis.app.provider.openai.OpenAIProvider) {
        ...
        val openAI = provider as? com.rikkaminis.app.provider.openai.OpenAIProvider
        ...
        if (openAI != null && isMediaOutput) {
            ...
            val prompt = genConfig.prompt
                ?: messages.lastOrNull { it.role == com.rikkaminis.app.data.model.LLMMessage.Role.USER }?.content?.takeIf { it.isNotEmpty() }
                ?: ""
    ...
    private fun executeStreamingRun(requestJson: String, dir: File) {
        ...
        try {
            ...
            val thinkingLevel = req.optString("thinking_level", "").let {
                if (it.isEmpty()) com.rikkaminis.app.data.model.ThinkingLevel.OFF
                else safeEnum(it, com.rikkaminis.app.data.model.ThinkingLevel.OFF)
            }
            ...
            for (i in 0 until arr.length()) {
                ...
                when (getString(p, "kind").lowercase()) {
                    "tooluse" -> {
                        ...
                        parts.add(com.rikkaminis.app.data.model.AgentContentPart.ToolUse(id = id, name = name, input = arguments))
                    }
                    "toolresult" -> {
                        val id = getString(p, "toolUseId").ifBlank { getString(p, "id") }
                        val name = getString(p, "name")
                        ...
                        parts.add(com.rikkaminis.app.data.model.AgentContentPart.ToolResult(
                            id = id,
                            name = name,
                            content = content,
                            isError = error,
                    ...
                    "image", "imagedata" -> {
                        ...
                        parts.add(com.rikkaminis.app.data.model.AgentContentPart.ImageData(data = data, mimeType = mimeType, linuxPath = linuxPath))
                    }
                    "text" -> parts.add(com.rikkaminis.app.data.model.AgentContentPart.Text(getString(p, "text")))
                }
            }
        }
        return parts
    ...
    private fun parseToolsJson(arr: JSONArray?): List<com.rikkaminis.app.data.model.AgentToolDefinition> {
        if (arr == null) return emptyList()
        val out = mutableListOf<com.rikkaminis.app.data.model.AgentToolDefinition>()
        for (i in 0 until arr.length()) {
            val t = arr.getJSONObject(i)
            val params = linkedMapOf<String, com.rikkaminis.app.data.model.AgentToolParam>()
            t.optJSONObject("parameters")?.let { ps ->
                ps.keys().forEach { k ->
                    val v = ps.getJSONObject(k)
                    params[k] = com.rikkaminis.app.data.model.AgentToolParam(
            ...
            out.add(com.rikkaminis.app.data.model.AgentToolDefinition(
     ...
     * thinking rules into [com.rikkaminis.app.provider.thinking.ThinkingRuleResolver]'s
    ...
    private fun restoreThinkingRules(req: JSONObject) {
        val count = com.rikkaminis.app.provider.thinking.ThinkingRuleResolver
