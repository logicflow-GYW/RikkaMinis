...
data class LLMModel(
    ...
    // Mirrors iOS LLMModel.declaresNoEffortTiers. Distinct from
    // `reasoningEffortValues == null`, which also covers "the catalog has never
    // heard of this model". Nullable so decoding a model persisted before this
    // field existed yields null ("unknown", pre-existing behaviour).
