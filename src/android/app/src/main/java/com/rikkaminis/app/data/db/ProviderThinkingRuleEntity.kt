...
/**
 * [T-android-thinking-rules-phase2] One user-authored thinking rule, scoped to a single
 * provider instance. Ported from upstream OpenMinis (parity with iOS 4d7fb9b4).
 *
 * Only CUSTOM rules are persisted — built-in vendor rules are compile-time constants in
 * [com.rikkaminis.app.provider.thinking.ThinkingRuleResolver.builtInRules] and are never
 * stored. The resolver reads a provider's persisted rows in [sortOrder] order and
 * prepends them ABOVE the built-in list, so a custom rule can override a vendor default
 * by matching first, but can never remove a built-in.
 *
...
data class ProviderThinkingRuleEntity(
    /** Stable UUID assigned at creation; the drag-reorder target. */
    ...
    /** Serialized [com.rikkaminis.app.provider.thinking.ThinkingWireFormat]; null = "no opinion". */
