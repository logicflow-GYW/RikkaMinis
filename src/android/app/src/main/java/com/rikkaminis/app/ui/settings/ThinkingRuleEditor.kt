                ...
                when (choice) {
                    FormatChoice.REASONING_EFFORT, FormatChoice.REASONING_EFFORT_NESTED -> {
                        Row(
                            ...
                            Switch(
                                checked = sendOffValue,
                                onCheckedChange = { sendOffValue = it },
                            )
                        }
                        if (sendOffValue) {
                            OutlinedTextField(
...
fun wireFormatSummary(fmt: ThinkingWireFormat?): String = when (fmt) {
    ...
    is ThinkingWireFormat.DeepSeekSibling -> "thinking + reasoning_effort (official)"
    is ThinkingWireFormat.DeepSeekRelay -> "reasoning_effort (relay-safe)"
    is ThinkingWireFormat.QwenDual -> "enable_thinking + budget"
    is ThinkingWireFormat.QwenRootOnly -> "reasoning_effort (relay-safe)"
