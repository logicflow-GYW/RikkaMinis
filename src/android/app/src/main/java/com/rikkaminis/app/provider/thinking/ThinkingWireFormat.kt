 ...
 * [com.rikkaminis.app.provider.openai.OpenAIProvider]). Gemini and Anthropic have their
...
sealed interface ThinkingWireFormat {
    ...
    /**
     * [T-deepseek-v4-official-only] DeepSeek V4 served by a third-party
     * OpenAI-compatible relay (NOT api.deepseek.com). The relay rejects the
     * vendor-internal `thinking:{type, reasoning_effort}` with UNKNOWN_FIELD, so ON is
     * the standard top-level `reasoning_effort`; OFF must STILL be the explicit
     * `thinking:{type:"disabled"}` because deepseek-v4 thinks BY DEFAULT and omitting
     * the toggle leaves it on. Distinct from [ReasoningEffort], whose OFF path sends an
     * off-tier string (or omits) rather than the disabled toggle. Absorbed by RikkaMinis
     * from the pre-refactor `isOfficialDeepSeek` branch (7aea092d era).
     */
    data object DeepSeekRelay : ThinkingWireFormat
    ...
    /**
     * [T-qwen-thinking-private-fields-host-gated] Qwen family on a NON-DashScope endpoint
     * (a relay). Qwen thinks by DEFAULT, so OFF must send an explicit
     * `enable_thinking: false` (omission stalls the visible stream) — but the
     * DashScope-private `thinking_budget` / `extra_body` are NOT portable: a strict
     * OpenAI-compatible relay (e.g. tokenrhythm.studio) 400s on them with UNKNOWN_FIELD.
     * So ON falls back to the standard top-level `reasoning_effort` (the only thinking
     * control such a relay accepts), while OFF keeps the portable `enable_thinking: false`.
     *
     * Distinct from [QwenDual], which is reserved for the DashScope endpoint itself.
     * Absorbed by RikkaMinis from fix 7aea092d (host-gate) + the iOS `qwenRootOnly`
     * vocabulary; the iOS ON shape differs (enable_thinking:true) because iOS never
     * measured a relay's ON field the way Android did.
     */
    data object QwenRootOnly : ThinkingWireFormat
