...
internal suspend fun ChatViewModel.executeSpawnAgentTool(argsJson: String): ToolExecutionResult {
    ...
    return com.rikkaminis.app.ui.chat.runSubagentLoop(
        skillName = skillName,
        query = query,
        ...
        streamProvider = { messages ->
            ...
            com.rikkaminis.app.sandbox.offload.ProviderExecutionGateway.stream(
                context = context,
                instance = instance,
...
internal fun ChatViewModel.maybeReloadSkillsForPath(argsJson: String) {
    com.rikkaminis.app.ui.chat.maybeReloadSkillsForPath(argsJson, skillRepository)
}
...
internal suspend fun ChatViewModel.executeBrowserUseTool(argsJson: String): ToolExecutionResult =
    com.rikkaminis.app.ui.chat.executeBrowserUseTool(
        argsJson = argsJson,
...
internal fun ChatViewModel.persistBrowserArtifact(filename: String, data: ByteArray): String? =
    com.rikkaminis.app.ui.chat.persistBrowserArtifact(activeSessionId, filename, data, context)

internal fun ChatViewModel.linuxPathToMinisURL(path: String): String? =
    com.rikkaminis.app.ui.chat.linuxPathToMinisURL(path)
...
internal fun ChatViewModel.executeMemoryWriteTool(argsJson: String): ToolExecutionResult {
    ...
    return com.rikkaminis.app.ui.chat.executeMemoryWriteTool(argsJson, repo, _memoryEnabled.value) { record ->
...
internal fun ChatViewModel.executeMemoryGetTool(argsJson: String): ToolExecutionResult {
    val repo = memoryRepository ?: return ToolExecutionResult("Error: Memory not available", false)
    return com.rikkaminis.app.ui.chat.executeMemoryGetTool(argsJson, repo) { record ->
        _memoryToolRecords.value = _memoryToolRecords.value + record
    }
}
...
internal fun ChatViewModel.executeMemoryRollupTool(): ToolExecutionResult {
    val repo = memoryRepository ?: return ToolExecutionResult("Error: Memory not available", false)
    return com.rikkaminis.app.ui.chat.executeMemoryRollupTool(repo) { record ->
        _memoryToolRecords.value = _memoryToolRecords.value + record
    }
}
 ...
 */
internal fun ChatViewModel.determineIntegrationTier(reqs: com.rikkaminis.app.data.repository.SkillRepository.SkillRequirements?): Int {
 ...
 * Queries [com.rikkaminis.app.data.repository.EnvVarRepository] which is
...
internal fun ChatViewModel.envVarsSnapshot(): Map<String, String> =
    try {
        ...
        val repo = com.rikkaminis.app.data.EnvVarRedactor.envVarRepository
            ?: com.rikkaminis.app.data.repository.EnvVarRepository(context)
        repo.allAsDict()
    } catch (e: Exception) {
...
internal fun ChatViewModel.buildSystemPrompt(): String? {
    ...
    val identitySection = com.rikkaminis.app.agent.SystemPromptBuilder.identitySection(context)
